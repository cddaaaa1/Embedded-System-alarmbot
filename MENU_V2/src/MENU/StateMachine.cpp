...
void handleInput(char c)
{
    ...
    case ALARM_MENU:
        ...
        case 'L':
            alarmMenuIndex--;
            if (alarmMenuIndex < 0)
                alarmMenuIndex = ALARM_SUB_COUNT - 1;
            break;
        // R => 光标下
        case 'R':
            alarmMenuIndex++;
            if (alarmMenuIndex >= ALARM_SUB_COUNT)
                alarmMenuIndex = 0;
            break;
        // C => 确认
        case 'C':
    ...
    case ALARM_SET_COUNTDOWN:
    {
        ...
        case 'L':
            alarmFocus--;
            if (alarmFocus < 0)
                alarmFocus = FOCUS_COUNT - 1;
            break;
        case 'R':
            alarmFocus++;
            if (alarmFocus >= FOCUS_COUNT)
                alarmFocus = 0;
            break;
    ...
    case TODO_MENU:
    {
        ...
        case 'C':
            ...
            if (todoListCursor >= 0 && todoListCursor < todoCount)
            {
                ...
                toggleTodoStateByName(todoList[todoListCursor].text);
            }
            break;
